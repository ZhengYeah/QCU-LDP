...
class RobustRadiusSKLearn:
    def __init__(self, model, x, perturb_col, omega, tau, p=float('inf')):
        ...
        :param x: (dataframe) input to the model, sometimes it is a dataframe
    ...
    def form_hyper_rectangle(self, step_size, rate):
        ...
        left_direction = np.random.binomial(1, rate, len(self.perturb_index))
        right_direction = np.random.binomial(1, rate, len(self.perturb_index))
        # update the rectangle
        rect[0] = rect[0] - left_direction * step_size
        rect[1] = rect[1] + right_direction * step_size
        # clip the rectangle
        start_rect = [np.clip(x, 0, 1) for x in rect]
        return rect
